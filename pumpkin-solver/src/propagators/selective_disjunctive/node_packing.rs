...
impl<Var: IntegerVariable + 'static> Propagator for NodePackingPropagator<Var> {
    ...
    fn debug_propagate_from_scratch(&self, _context: PropagationContextMut) -> PropagationStatusCP {
        // Since it is a heuristic approach, it is difficult to reproduce
        todo!();
    }
...
impl<Var: IntegerVariable + Clone + 'static> NodePackingPropagator<Var> {
    ...
    fn find_conflict(
        ...
        // First we look for pairwise conflicts
        let pairwise_conflict = self.find_pairwise_conflict(context)?;
        if pairwise_conflict.is_some() {
            return Ok(pairwise_conflict);
        }

        // Then we heuristically try to find larger conflicts
        let conflict = self.find_clique_heuristic(context)?;
        if conflict.is_some() {
            return Ok(conflict);
        }
