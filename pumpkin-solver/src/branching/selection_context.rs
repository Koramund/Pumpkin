...
pub struct SelectionContext<'a> {
    ...
    pub(crate) extended_literals: &'a HashSet<u32>,
}

impl<'a> SelectionContext<'a> {
    pub fn new(assignments: &'a Assignments, rng: &'a mut dyn Random, counters: &'a mut SolverStatistics, extended_literals: &'a mut HashSet<u32>) -> Self {
        SelectionContext {
            assignments,
    ...
    pub fn are_all_variables_assigned(&self) -> bool {
        self.assignments
            .get_domains_cloned().iter()
            .all(|domain_id| !domain_id.decidable || self.assignments.is_domain_assigned(domain_id))
    }
